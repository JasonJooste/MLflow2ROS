...
def build_validator_container(scope="session"):
    ...
    subprocess.run(
        [
            ...
            "--tag",
            VALIDATOR_CONTAINER,
            Path.cwd() / "tests" / "integration" / "test_nodes",
        ],
        check=True,
        text=True,
    )
...
def mlflow_server_container(docker_client, scope="session"):
    container = docker_client.containers.run(
        ...
        "mlflow server --host 127.0.0.1 --port 55555",
        network="host",
        detach=True,
        auto_remove=True,
    )
...
def make_image_and_run_tests(docker_client, test_name, model_name):
    ...
    app.make_image(
        ...
        model_ver=1,
        tag=model_name,
        output_directory=Path("tests_generated_files") / test_name,
    )

    model = docker_client.containers.run(
        model_name, network="host", detach=True, auto_remove=True
    )
...
def test_sklearn_tensor_basic(
    docker_client,
    mlflow_server_container,
    roscore_container,
    build_validator_container,
    override_tracking_uri,
):
